				...
				onClose: function(oAction) {
					if (oAction == "YES") { 
						...
						var settings = {
							"async": true,
							"crossDomain": true,
							"url": "https://portal.jepco.com.jo/MeterReplacmentPortalApisPRD/api/TbMeterReplacment/UpdateStatus",
							"method": "POST",
							"headers": {
...
// 			atetime =  currentdate.getHours() + ":"   + currentdate.getMinutes() + ":" + currentdate.getSeconds();
			...
			$.ajax({
				type: "POST",
				url: "https://portal.jepco.com.jo/JepcoSAPProxyPRD/SapProxy/InsertMeterServicesSet",
				dataType: "json",
			...
			// 		console.log(xmlDoc)
			// 		sap.ui.core.BusyIndicator.hide();
			// 		//responseMsg = xmlDoc.getElementsByTagName("error")[0].getElementsByTagName("message")[0].childNodes[0].nodeValue;
			// 		MessageBox.error("حدث خطأ اثناء عملية التبديل");
			// 		//MessageBox.error(responseMsg);
		...
		onEditRead: function () {
			...
			if (flag) {
				var settings = {
					"async": true,
					"crossDomain": true,
					"url": "https://portal.jepco.com.jo/MeterReplacmentPortalApisPRD/api/TbMeterReplacment/UpdateReadings",
					"method": "POST",
					"headers": {
						"content-type": "application/json",
						"cache-control": "no-cache",
				...
				$.ajax(settings).done(function (response, status) {
					...
					oModel.setData(data);
					oTable.setModel(oModel);
					MessageBox.show("تمت تعديل القراءات بنجاح");
