sap.ui.define([
	...
	Controller,BusyIndicator, MessageBox, Sorter, Filter, FilterOperator, FilterType, JSONModel, Dialog, Button, ButtonType
) {
	"use strict";
 ...
 onQueryRound: function(){
	...
	var rsURL = "https://portal.jepco.com.jo/JepcoSAPProxyPRD/SapProxy/GetDuplicates";
	var rond = this.getView().byId("teamRound").getValue();
	//var usr  = this.getView().byId("teamName").getValue();
	var usr  = this.getView().byId("teamName").getSelectedItem().mProperties.text;
	usr = usr.toUpperCase();
	if (rond == '' && usr == '') {
 ...
 onDeleteRound: function(){
	var ui5UserApp =  sap.ui.getCore().getModel().getProperty("/globalData/oUserName");
	var engUserName = "RONDD-" + ui5UserApp;
	var rsURL = "https://portal.jepco.com.jo/JepcoSAPProxyPRD/SapProxy/InsertTechNote";
	var rond = this.getView().byId("teamRound").getValue();
	//var usr  = this.getView().byId("teamName").getValue();
	var usr  = this.getView().byId("teamName").getSelectedItem().mProperties.text;
	if (rond == '' || usr == '') {
		MessageBox.alert("الرجاء ادخال اسم المستخدم للفرقة الفنية ورقم الجولة	");
		return;
	}
	let that = this;
	$.ajax({
 ...
 onNotePicPress: function(oEvent) { 
		...
		var techNoteId = this.getView().byId("idTechnicalNotes").getModel().oData[lineNumber].id;
		var noteURL = "https://portal.jepco.com.jo/MeterReplacmentPortalApisPRD/api/TbTechnicalNotesAndIncident/GetNotesByID?ID=" + techNoteId + "";
		let that = this;
		$.ajax({
			type: 'GET',
			url: noteURL ,
			...
			contentType: "application/json",
	 	    success: function(data, status, response){
				  ...
				  var flexbox = new sap.m.HBox({
					  ...
					  items: [  new sap.m.Image({  src: src1,  
						                           alt: "Tech Note image",  
												   height : "500px",
												   mode : "Image",
												   width : "600px" }) ]
...
///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////			
			...
			for(let i=0; i< engTechUser.length;i++){
			 ...
			 let oUserID =  sap.ui.getCore().getModel().getProperty("/globalData/oUserName");
		...
		//	 let url = "http://85.159.218.156:8000/sap/opu/odata/sap/ZCOMMDTL_SRV/ZENG_USERSet?$filter=startswith(EngTechUser,'" + payload + "')&$format=json"
			 $.ajax({
				 type: 'POST',
				 url: "https://portal.jepco.com.jo/JepcoSAPProxyPRD/SapProxy/GetEng_MeterSet",
				 dataType: "json",
				 contentType: "application/json",
				 data: JSON.stringify(
					 {
			...
			var settings = { 
				"async": true,
				"crossDomain": true,
				"url": "https://portal.jepco.com.jo/MeterReplacmentPortalApisPRD/api/TbTechnicalNotesAndIncident/GetNotesByUi5UserIds",
				"method": "POST",
				"headers": {
					"content-type": "application/json",
					"cache-control": "no-cache",
			...
			$.ajax(settings).done(function (response) {
				...
				for (var key in response) {
					if (response.hasOwnProperty(key)) {
						response[key].createdDate = response[key].createdDate.split('T')[0];
		...
		onShowMore: function (oEvent) {
			...
			var that = this;
			let urlByID = "https://portal.jepco.com.jo/MeterReplacmentPortalApisPRD/api/TbMeterReplacment/GetMeterReplacmentByID?ID=" + oBindingObject.id + "";
			$.ajax({
				type: 'GET',
				url:   urlByID,
				dataType: "json",
		...
		//  },		
		 onSearchMeter: function(oEvent){ 
			...
			if(oEvent.getParameter("id").split("--").pop() == "searchOldMeter"){
				var Searcholdmeter1 = this.getView().byId("searchOldMeter").getValue();
				 oflg = 1;
			}
			...
			if(oEvent.getParameter("id").split("--").pop() == "searchOldMeterAll"){
				var Searcholdmeter1 = this.getView().byId("searchOldMeterAll").getValue();
				 oflg = 1;
			}
			if(oEvent.getParameter("id").split("--").pop() == "searchNewMeterAll"){
				var Searcholdmeter1 = this.getView().byId("searchNewMeterAll").getValue();
				 oflg = 0;
			}
			...
			var settings = {
				"async": true,
				"crossDomain": true,
				"url": "https://portal.jepco.com.jo/MeterReplacmentPortalApisPRD/api/TbMeterReplacment/GetMeterReplacmentByMeterNumber?MeterNumber="+Searcholdmeter1+"&OldMeterFalg="+oflg,
				"method": "GET",
				"headers": {
					"content-type": "application/json",
					"cache-control": "no-cache",
			...
			if(lv_oEvent == "searchOldMeterAll" || lv_oEvent == "searchNewMeterAll" ) {
		...
		onSort: function () {
			...
			oView.byId("idTableOffline").getBinding("items").sort(sOrder && new Sorter("status", sOrder === "desc"));
			oView.byId("idTableOtherMeter").getBinding("items").sort(sOrder && new Sorter("status", sOrder === "desc"));
		...
		onSortByDate: function(){
			...
			oView.getModel("appView").setProperty("/order", iOrder); 
			oView.byId("idTechnicalNotes").getBinding("items").sort(sOrder && new Sorter("createdDate", sOrder === "asc"));
				////////////////////////////////////////////////////
		},
		...
		onSortUi5User: function () {
			...
			oView.getModel("appView").setProperty("/order", iOrder);
			oView.byId("idTechnicalNotes").getBinding("items").sort(sOrder && new Sorter("ui5User", sOrder === "desc")); 
		...
		onSortUser: function () {
			...
			oView.getModel("appView").setProperty("/order", iOrder);
			oView.byId("idTableOffline").getBinding("items").sort(sOrder && new Sorter("ui5User", sOrder === "desc"));
			oView.byId("idTableOtherMeter").getBinding("items").sort(sOrder && new Sorter("ui5User", sOrder === "desc"));
		...
		onCreatedDate: function () {
			...
			oView.getModel("appView").setProperty("/order", iOrder);
			oView.byId("idTableOffline").getBinding("items").sort(sOrder && new Sorter("createdDate", sOrder === "desc"));
			oView.byId("idTableOtherMeter").getBinding("items").sort(sOrder && new Sorter("createdDate", sOrder === "desc"));
		},
